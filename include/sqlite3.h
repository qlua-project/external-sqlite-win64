...
*/
SQLITE_API int SQLITE_APICALL sqlite3changeset_upgrade(
  sqlite3 *db,
  const char *zDb,
  int nIn, const void *pIn,       /* Input changeset */
  int *pnOut, void **ppOut        /* OUT: Inverse of input */
...
**   the current document. If successful, (SQLITE_CALLBACK *pz) is set to point to a buffer
...
**   output variable (SQLITE_CALLBACK *ppToken) is set to point to a buffer containing the
**   matching document token, and (SQLITE_CALLBACK *pnToken) to the size of that buffer in
**   bytes.
**
...
**   locale, and output parameters (SQLITE_CALLBACK *pzLocale) and (*pnLocale) are set
**   to NULL and 0, respectively. However, if the fts5_locale() function
**   was used to associate a locale with the value when it was inserted
**   into the fts5 table, then (SQLITE_CALLBACK *pzLocale) is set to point to a nul-terminated
**   buffer containing the name of the locale in utf-8 encoding. (SQLITE_CALLBACK *pnLocale)
