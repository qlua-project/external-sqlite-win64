...
set __EXTENSIONS=
rem -DSQLITE_ENABLE_FTS5 ^
rem -DSQLITE_ENABLE_FTS4 ^
rem -DSQLITE_ENABLE_FTS3
set __FEATURES=^
...
METADATA=0 ^
EXT_FEATURE_FLAGS= ^
"OPT_XTRA=" "OPTIONS=" "OPTS=%__EXTENDED_WIN10% %__RECOMMENDED% %__MY% %__EXTENSIONS%"
rem OPT_FEATURE_FLAGS= ^
...
::                "OPTS=%__OPTS%"
nmake /f Makefile.msc shell USE_RC=0 ^
        %NMAKE_OPTS%
        rem %NMAKE_OPTS_DEFAULT% ^
mkdir %~dp0bin
...
rem echo nmake /f Makefile.msc dll USE_RC=0 ^
rem         %NMAKE_OPTS%
nmake /f Makefile.msc dll USE_RC=0 ^
        %NMAKE_OPTS%
        rem %NMAKE_OPTS_DEFAULT% ^
